...
def _restore(model: Module, backup: dict) -> None:
...
def _save(
    model: Module, adv_param: str, adv_eps: float, backup: dict, backup_eps: dict
) -> None:
    for name, param in model.named_parameters():
...
def _attack_step(
    model: Module, adv_param: str, adv_lr: float, backup_eps: dict
) -> None:
    e = 1e-6
    for name, param in model.named_parameters():
        if param.requires_grad and param.grad is not None and adv_param in name:
            ...
            if norm1 != 0 and not torch.isnan(norm1):
                ...
                param.data = torch.min(
                    torch.max(param.data, backup_eps[name][0]), backup_eps[name][1]
                )
...
def _attack_backward(
    ...
    with torch.cuda.amp.autocast(enabled=apex):
        ...
        inputs = {k: v for k, v in batch.items() if k in model.model_forward_args}
        labels = {k: v for k, v in batch.items() if k == "labels"}["labels"]
        y_preds = model(inputs)
        adv_loss = model.loss(y_preds, batch)
...
class AWP(Algorithm):
    ...
    def match(self, event, state):
        return (
            ...
            and state.timestamp.epoch >= self.start_epoch
        )
